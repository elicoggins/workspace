...
fn restore_window_group(jobs: &[RestoreJob<'_>], options: RestoreOptions) -> Vec<Result<()>> {
    ...
    if first.bundle_id.as_deref() == Some("com.google.Chrome") {
        let windows: Vec<_> = jobs.iter().map(|job| job.window).collect();
        return match chrome::restore_windows(&windows) {
            Ok(true) => {
                wait_for_window_count(first, jobs.len(), options);
                let _ =
                    set_group_frames_with_retry(first, jobs.len(), &targets, current_pids(first));
